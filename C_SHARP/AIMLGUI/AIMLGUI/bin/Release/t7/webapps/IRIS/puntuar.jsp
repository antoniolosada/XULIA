...
<%
        ...
        <%
                            ...
                            out.println("<td color= white bgcolor = powderblue align=center width=40><b>"+String.valueOf(rs.getString("num_dorsal")+"</b></td>"));
        %>
                                    <td id="puesto_tabla<%=iPosDorsal%>" color= white bgcolor = white align=center  width="20">
                                    <input type="button" name="puesto<%=iPosDorsal%>" id="<%=iPosDorsal%>" onclick='DorsalNoPresente(this);' value='<%=rs.getString("puesto")%>' style='BORDER: rgb(128,128,128) 0px solid; BACKGROUND-COLOR: rgb(255,255,255); height:25px; width:20px'">
                                    </td>
                                        <td>
                                            <font size = 2>
        ...
        <%
                            for (i=1; i<=9; i++)
                            {
                               ...
                               if (i <= iNumDorsales)
                                    out.println("<td><input type = 'button' name ='p"+iPosDorsal+"_"+i+"' id ='"+iPosDorsal+"_"+i+"' value = '"+i+"' ondblclick='pasa(this);' onclick='pasa(this);' align=center width=40 style='BORDER: rgb(128,128,128) 1px solid; BACKGROUND-COLOR: rgb(255,255,255); height:25px; width:25px'></td>");
                               else if (i == iNumDorsales+1)
                                    out.println("<td><input type = 'button' name ='c"+iPosDorsal+"' id ='"+iPosDorsal+"' value = 'C' onclick='cambio(this,"+i+",9);' align=center width=40 style='BORDER: rgb(128,128,128) 1px solid; BACKGROUND-COLOR: yellowgreen; height:25px; width:25px'></td>");
...
<%
        ...
        <%
                        ...
                        while(rs.next()){
                            ...
                            out.println("<input type = 'button' name ='"+sNumDorsal+"' id ='"+iPosDorsal+"' value = '"+sNumDorsal+"' onclick='pasaElim(this,1);' align=center width=45 style='BORDER: rgb(128,128,128) 1px solid; BACKGROUND-COLOR: white; height:20px; width:30px'>");
                            out.println("</td>");
                            iPosDorsal++;
                            if (iPosDorsal % 6 == 0)
                                out.println("</tr><tr>");
    ...
    <%
        if (iFase == 1)
            out.println("<input type='button' name='Send' value='Ok' onclick='Enviar();'>");
        else
            out.println("<input type='button' name='Send' value='Ok' onclick='EnviarElim();'>");
    %>

    <%
        if (iFase > 1)
            out.println("<input type='button' name='Dorsales' value='"+iMarcas+"'>");
    %>
    
    <%=sJuez%>.<%=sBaileAbrev%> - <%=sCategoria.substring(0, 10) %>
   <input type="button" name="R" value="R" onclick='Recargar();'> 
   <b></b></form>
    </td></tr>
    </table>
<%
