...
namespace AIMLGUI
{
    public class ProcesamientoComandos
    {
        ...
        DictationGrammar GramaticaDictado;
        public SpeechRecognitionEngine recognizer;
        public SpeechRecognizer recognizerW;
        ...
        // Métodos ******************************************************************************************************************************************************
        #region iniciacion
        public void ActivarReconocimientoVoz(ProcesamientoComandos Com, aimlForm Form, frmEstado FormEstado, RichTextBox Salida)
        {
            /******************      IMPORTANTE   *******************************************************
            ...
            try
            {
                bool TomcatArrancado = true;
                //Arrancamos Tomcat
                if (ArranqueTomcat != "")
                    if (!BuscaTituloVentana("Tomcat"))
                    {
                        Process p = new Process();
                        p.StartInfo.FileName = "cmd";
                        p.StartInfo.Arguments = " /c \"" + SustituirDirectorios(ArranqueTomcat) + "\" " + SustituirDirectorios(ArranqueTomcat_unidad) + " " + SustituirDirectorios(ArranqueTomcat_parametros);
                        //p.StartInfo.CreateNoWindow = true;
                        p.StartInfo.WindowStyle = ProcessWindowStyle.Minimized;
                        p.Start();
                        if (EsperaTituloVentana("Tomcat", TiempoArranqueTomcat * 1000))
                        {
                            ActivarVentanaTitulo("Tomcat");
                            MaximizarMinimizarVentanaActiva(SW_SHOWMINIMIZED);
                        }
                        else
                        {
                            LOG("No ha sido posible arrancar tomcat");
                            TomcatArrancado = false;
                        }
                    }

                if ((ModoComandoGoogle == "S") && TomcatArrancado)
                {
                    Thread.Sleep(EsperaArranqueTomcatModoComandoGoogle*1000);
                    if (EjecutarChromeRecVoz(IdiomaComandoGoogle, CodigoIdiomaComandoGoogle, 1000) == null)
                    {
                        LOG("ERROR FATAL: no se ha podido ejecutar Google Chrome para reconocimiento de comandos");
                    }
                }
            }
            catch (Exception e)
            {
                LOG("Error arrancando Tomcat" + e.Message);
            }
        }
        ...
        { //Carga inicial de las gramáticas disponibles en el arranque
            ...
            if (!frmAIML.ActivarSAPI4)
                GramaticaDictado = new DictationGrammar();
        ...
        { 
            ...
            {
                LOG("ERROR en RecuperarElementosGramatica: " + e.Message);
                string[] elementos = lista.ToArray();
                return elementos;
            }
        }
                ...
                foreach (string gramatica in Gramaticas)
                {
                    ...
                    { //La gramática nunca ha sido creada
                        ...
                        if (ListaElementos.Length > 0)
                        {
                            ...
                            { //No es necesario cargar las gramáticas
                            }
        ...
        {
            if (RV_Windows)
                recognizerW.LoadGrammar(GramaticaDictado);
            else
        ...
        {
            if (RV_Windows)
                recognizerW.UnloadGrammar(GramaticaDictado);
            else
        ...
        public string TextoReconocido(string texto, float GradoPrecision, bool ModoDictadoIdiomas)
        {
            try
            {
                ...
                txtSalida.AppendText(salida + " (" + GradoPrecision.ToString().Substring(0, 5) + "%)" + Environment.NewLine);
                ...
                if (!XULIA_ACTIVA)
                    if ((salida != XuliaActivar) && (salida != XuliaAtencion))
                    {
                ...
                if (Strings.InStr(this.MODO, "·DICTADO") > 0)
                {
        ...
        public void EnviarTecla(string tecla)
        {
            try
            {
                if (ACENTO || ACENTO_CIRCUNFLEJO || ACENTO_GRAVE || DIERESIS)
                {
                    if (EsVocal(tecla))
                    {
                ...
                //Desactivar acentos
                ACENTO = ACENTO_GRAVE = ACENTO_CIRCUNFLEJO = DIERESIS = false;
        ...
        {
            return BuscarComandoEx(GramaticaAplicacion, entrada, GradoPrecision, true);
        }
        sGramatica BuscarComandoEx(bool GramaticaAplicacion, string entrada, float GradoPrecision, bool GramaticaActiva)
        {
            ...
            try
            {
                List<GramaticaCargada> GramActivas = GramaticasCargadas.FindAll(x => ((x.Activa == GramaticaActiva) && (x.GramaticaAplicacion == GramaticaAplicacion)));
        ...
        void InterpretarComandoEnModo(sGramatica ElementoGram, string salida, bool ModoDictadoIdiomas)
        {
            try
            {
                ...
                else // ·DICTADO y ·CONVERSACION. No se ha reconocido un comando en las gramáticas activas pasamos a dictado y dictado google
                {
                    ...
                    else if (MODO == "·DICTADO")
                    {
                        EnviarEventosTeclas(salida);
                    }
                    else if ((Strings.InStr(MODO, "·DICTADO·") > 0) || (Strings.InStr(MODO, "·CONVERS·") > 0))
                    { //Dictado Google Chrome
        ...
        void ProcesarEntradaModoConversacion(string salida)
        {
            ...
            else
            {
                ...
                sGramatica cmd = Me.BuscarComandoEx(false, salida, (float)0.99, false);
                if (cmd.Nombre == null)
                {
                    cmd = Me.BuscarComandoEx(true, salida, (float)0.99, false);
                    if ((cmd.Nombre == null) && InteractuarConXulia)
                        frmAIML.ProcesarEntrada(salida);
                }
            }
        }
        ...
        public bool CambioModo(string Modo)
        {
            try
            {
                ...
                { //Dictado contínuo multi-idioma Google Chrome
                    ...
                    System.IO.File.WriteAllText(ArchivoComandos, "PARAR");
                    ...
                    if (!ChromeEjecutandose)
                        dc.proceso = EjecutarChromeRecVoz(Idioma, dc.codigo, 100);
                    DictadosActivos.Add(dc);

                    //Activamos el nuevo idioma parando todos los demás que pudieran estar activos
                    System.IO.File.WriteAllText(ArchivoComandos, "REANUDAR." + Idioma);
        ...
        void VolverModo(string actual, string nuevo)
        {
            try
            {
                ...
                if ((actual == "·DICTADO") || (actual == "·CONVERSACION")) //La descarga de la gramática de dictado tiene cfg propia
                {
                ...
                else if ((Strings.InStr(actual, "·DICTADO·") > 0) || (Strings.InStr(actual, "·CONVERS·") > 0))
        ...
        bool EjecutarComando(string comando, string[] parametros)
        {
            try
            {
                bool ejecutar = true;
                ...
                else if (Strings.InStr(comando, "[CERRAR_APP_CHROME]") > 0)
                {
                    try
                    {
                        while (true)
                        {
                            Interaction.AppActivate("RECVOZ.GOOGLE.");
        ...
        Process EjecutarChromeRecVoz(string Idioma, string CodigoIdiomaGoogle, int EsperaMS)
        {
            BuscarClaseAppActiva();

            Process p = new Process();
            p.StartInfo.FileName = "chrome";
            p.StartInfo.Arguments = "--app=" + URLBaseIdioma + "?idioma=" + Idioma + "&codigo=" + CodigoIdiomaGoogle + "&auto=S";
            p.StartInfo.WindowStyle = ProcessWindowStyle.Minimized;
            p.Start();

            System.IO.File.WriteAllText(ArchivoComandos, "PARAR");
            //Espero N segundos para comprobar que se ejecuta
        ...
        void InicializarReconocedorVoz()
        {
            ...
            {
                if (RV_Windows)
                    recognizerW = new SpeechRecognizer();
                else
        ...
        void ActivarReconocedorSAPI()
        {
            if ((!frmAIML.ActivarSAPI4) && (!(ModoComandoGoogle == "S")))
            {
                if (RV_Windows)
                {
                    ...
                    recognizerW.SpeechRecognized +=
                      new EventHandler<SpeechRecognizedEventArgs>(recognizer_SpeechRecognized);
                    recognizerW.Enabled = true;
